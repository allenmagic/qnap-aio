...
> # ⚠️ 本文已部分过时（2026-09-17 定单网关，2026-09-18 换 bridge）
>
> **① 2026-09-17：「同机双容器 + VRRP 浮动网关」的方案已在真机实测后放弃。**
> 原因：用隧道健康度驱动 VIP 漂移本身是故障源——实测两台 keepalived 启动相差
> 20+ 秒，抢到 VIP 的那台若隧道没连上，它的 DNS DNAT 指向不存在的 `10.251.1.1`，
> 全网 DNS 黑洞。改法不是"容器内降级"，而是**只剩一个网关**：main-router 同时做
> NAT + DHCP + 本机 DNS，DNS 上游按 `strict-order` 排成「隧道 DNS → 公网 DNS」，
> 客户端 DNS 始终是网关 `.1`，不再有 DNAT 目标切换这回事。
>
...
> §4.1 的性能对比、§6.2 side-router、§6.5~§6.6 dnsmasq 容器、§7（VRRP/浮动网关）、
> §15（WAN 双 DHCP 客户端）、附录 A、附录 B。
>
> **仍然有效**：§2 的设计原则、§6.7 的 DNS 链路思路（要经过网关才谈得上分流）、
> §8 数据路径、§11 dnsmasq 配置细节、§14 XDP 约束、§17 排查方法。
>
> 最新口径以 `CLAUDE.md`、`README.md` 与 `modules/` 下的实际代码为准；
> 本文作为设计决策记录保留，全文尚未重写。
